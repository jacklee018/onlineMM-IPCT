...
public class KState<C extends StateCandidate<C, T, S>, T extends StateTransition, S extends Sample>
    ...
    //链表sequence中元素的结构：Tuple<一个阶段的候选点集Set, 对应阶段的样本点>
    ...
    public Tuple<C, S> estimate2() {
        ...
        return new Tuple<>(estimate, sequence.peekLast().two());
    }
    ...
    public List<C> sequence() {
        ...
        //获取最后一阶段中过滤概率filtprob最大的候选点，再根据predecessor倒推回去获得最优路径
     ...
     * 获取带对应样本点的最优路径序列
     */
    public List<Tuple<C, S>> sequence2() {
        if (sequence.isEmpty()) {
            return null;
        }
        ...
        //获取最后一阶段中过滤概率filtprob最大的候选点，再根据predecessor倒推回去获得最优路径
        C kestimate = estimate();
        LinkedList<Tuple<C, S>> ksequence = new LinkedList<>();

        for (int i = sequence.size() - 1; i >= 0; i--) {
            ksequence.push(new Tuple<>(kestimate, sequence.get(i).two()));
            kestimate = kestimate.predecessor();
        }
