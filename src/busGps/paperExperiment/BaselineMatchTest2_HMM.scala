...
import MapMatchBaseline.HMMM.HMMMatcher
...
object BaselineMatchTest2_HMM {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("HMMMatcher")
    val matcherBC: Broadcast[ClientMapMatcher[HMMMatcher]] = sc.broadcast(new ClientMapMatcher[HMMMatcher](mapPath, 50, classOf[HMMMatcher]))
    ...
    rate.foreach(t => {
      ...
      println(HMMMatcher.candsNum.get(), HMMMatcher.sampleNum.get())
      val candsNum = HMMMatcher.candsNum.get() - accuCandsNum
      val sampleNum = HMMMatcher.sampleNum.get() - accuSampleNum
      accuCandsNum = HMMMatcher.candsNum.get()
      accuSampleNum = HMMMatcher.sampleNum.get()
  ...
  def doBfMatch(gpsDataArray: mutable.Buffer[Array[String]], matcher: ClientMapMatcher[HMMMatcher]): Tuple3[Int, Int, Long] = {
