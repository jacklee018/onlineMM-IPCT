...
import MapMatchBaseline.STMatching.STMatcher
...
object BaselineMatchTest2_ST {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("STMatcher")
    val matcherBC: Broadcast[ClientMapMatcher[STMatcher]] = sc.broadcast(new ClientMapMatcher[STMatcher](mapPath, 50, classOf[STMatcher]))
    ...
    rate.foreach(t => {
      ...
      println(STMatcher.candsNum.get(), STMatcher.sampleNum.get())
      val candsNum = STMatcher.candsNum.get() - accuCandsNum
      val sampleNum = STMatcher.sampleNum.get() - accuSampleNum
      accuCandsNum = STMatcher.candsNum.get()
      accuSampleNum = STMatcher.sampleNum.get()
  ...
  def doBfMatch(gpsDataArray: mutable.Buffer[Array[String]], matcher: ClientMapMatcher[STMatcher]): Tuple3[Int, Int, Long] = {
