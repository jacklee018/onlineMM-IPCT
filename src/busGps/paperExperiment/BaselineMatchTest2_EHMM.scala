...
import MapMatchBaseline.HMMM.EnhanceHMMMatcher
 ...
 */
object BaselineMatchTest2_EHMM {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("EnhanceHMMMatcher")
    val matcherBC: Broadcast[ClientMapMatcher[EnhanceHMMMatcher]] = sc.broadcast(new ClientMapMatcher[EnhanceHMMMatcher](mapPath, 50, classOf[EnhanceHMMMatcher]))
    ...
    rate.foreach(t => {
      ...
      println(EnhanceHMMMatcher.candsNum.get(), EnhanceHMMMatcher.sampleNum.get())
      val candsNum = EnhanceHMMMatcher.candsNum.get() - accuCandsNum
      val sampleNum = EnhanceHMMMatcher.sampleNum.get() - accuSampleNum
      accuCandsNum = EnhanceHMMMatcher.candsNum.get()
      accuSampleNum = EnhanceHMMMatcher.sampleNum.get()
  ...
  def doBfMatch(gpsDataArray: mutable.Buffer[Array[String]], matcher: ClientMapMatcher[EnhanceHMMMatcher]): Tuple3[Int, Int, Long] = {
